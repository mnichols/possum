    ...
    .enclose(function(){
        ...
        function processInnerQueues(context) {
            ...
            return Promise.resolve(this.queues || [])
                .each(function(queue){
                    //we are re-using the context here...is that appropriate?
                    //it causes a bug b/c the context is COMPLETED by the inner queue process call
                    //while the outer queue is still processing
        ...
        stampit.mixIn(this, {
            process: function(context, messages) {
                ...
                debug(this.name,'processing',messages.length,'messages')
