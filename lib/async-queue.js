'use strict';

var stampit  = require('stampit')
   ,EventEmitter2 = require('eventemitter2').EventEmitter2
    ,Promise = require('bluebird')
    ,push = Array.prototype.push
    ,splice = Array.prototype.splice
    ,shift = Array.prototype.shift
    ,unshift = Array.prototype.unshift
    ,EVENTS = require('./system-events')
    ,event = require('./event')
    ,warn = require('debug')('possum:warn')
    ,debug = require('debug')('possum:debug:async-queue')

module.exports = queue

function queue(cfg) {

cfg = (cfg || {})

var emitter = (cfg.emitter || new EventEmitter2)
var contextCounter = 0

var processContext = stampit()
    .state({
        target: undefined
    })
    .enclose(function(){
        var cycles = 0
        if(!this.target) {
            throw new Error('target is required')
        }
        var currentMessage = undefined
            ,completed = false
            ,blacklist = [
                '_onExit'
                ,'_transition'
            ]

        function canCauseReplay(message) {
            return (message.inputType && blacklist.indexOf(message.inputType) < 0)
        }
        stampit.mixIn(this, {
            id: ++contextCounter
            ,completed: function(){
                completed = true
                return this
            }
            ,currentMessage: function(obj) {
                if(completed) {
                    throw new Error('context has completed. create another.')
                }
                if(!obj && !currentMessage) {
                    throw new Error('Possum is not in a processing context.')
                }
                if(obj) {
                    return (currentMessage = obj)
                }
                return currentMessage
            }
            ,infinite: function(){
                return (++cycles > 1000)
            }
            ,replayable: function(){
                var replay = !!currentMessage && canCauseReplay(currentMessage)
                return replay
            }
        })
        //seal the target
        Object.defineProperty(this, 'target',{
            writable: false
            ,configurable: false
        })
        //LSP to context.target
        Object.defineProperty(this, 'state',{
            get: function(){
                return this.target.state
            }
        })
    })


var model = stampit()
    .state({
        messages: []
        ,router: undefined
        ,queues: []
        ,name: 'possum.queue'
    })
    .methods(emitter)
    .methods({
        raise: function(e) {
            this.emit(e.topic, e)
            return this
        }
    })
    .methods({
        /**
         * Pushes either a single message or an {Array} of messages onto this queue
         * @method enqueue
         * */
        enqueue: function(msgs) {
            //accept a single message or an array of messages
            this.messages = this.messages.concat([].concat(msgs))
            return this
        }
    })
    .enclose(function(){

        function processInnerQueues(context) {
            //some messages should not trigger inner processing
            if(!context.replayable()) {
                return Promise.resolve(context)
            }
            debug('processing inner queues')
            return Promise.resolve(this.queues || [])
                .each(function(queue){
                    return queue.process.call(queue, context)
                })
                .then(function(){
                    return context
                })
        }
        function invoke(context, message, handler) {
            var fn = handler.fn
            if(!fn) {
                throw new Error('handler must have a `fn` to be invoked')
            }
            debug(this.name,'invoking',handler.name)
            return Promise.resolve(message)
                .bind(this)
                .then(function(msg) {
                    var result = fn.call(context.target, message.payload)
                    this.raise({
                        topic: EVENTS.QUEUE.HANDLED // -> marshal as 'handled'
                        ,action: handler.name
                        ,message: message
                    })

                    return result
                })
                .then(function(result){
                    this.raise({
                        topic: EVENTS.QUEUE.COMPLETED // -> marshal as 'completed'
                        ,action: handler.name
                        ,message: message
                    })
                    return processInnerQueues.call(this,context, message)
                })
        }


        function handle(context, message, handlers) {
            if(!handlers.length) {
                return Promise.resolve(context)
            }
            var handler = shift.call(handlers)
            return invoke.call(this, context, message, handler)
                .bind(this)
                .then(handle.bind(this,context, message, handlers))
        }

        function process(context, messages) {
            if(context.infinite()) {
                throw new Error('cycle detected in queue process for queue ' + this.name)
            }
            if(!messages.length) {
                return Promise.resolve(context)
            }
            var message = shift.call(messages)
                ,spec = {
                    inputType: message.inputType
                    ,state: context.state
                }
            context.currentMessage(message)

            var handlers = [].concat(this.router(spec))
            if(!handlers || !handlers.length) {
                debug(this.name,'NO HANDLERS for',spec.inputType,'at',spec.state)
                this.raise({
                    topic: EVENTS.QUEUE.NO_HANDLER
                    ,message: message
                    ,action: undefined
                })
                return process.call(this,context,  messages)
            }
            debug(this.name,'processing',message.inputType,'with',handlers.length,'handlers')
            return handle.call(this,context, message, handlers)
                .bind(this)
                .then(process.bind(this,context, messages))
        }

        var processing

        stampit.mixIn(this, {
            process: function(context, messages) {
                var self = this
                if(!context) {
                    throw new Error('a context is required')
                }
                if(!context.target) {
                    context = this.createContext(context)
                }
                if(processing && processing.context.id !== context.id) {
                    console.log('scheduling',context.id)
                    processing = {
                        context: context
                        ,inproc: processing.inproc.then(function(result){
                            console.log('running scheduled')
                            return self.process.call(self,context, messages)
                        })
                    }
                    return processing.inproc
                }
                if(messages) {
                    messages = [].concat(messages)
                } else {
                    messages = this.messages.splice(0, this.messages.length)
                }
                debug(this.name,'process',messages.length,'messages')

                processing = {
                    context: context
                    ,inproc: process.call(this,context,messages)
                        .then(function(result){
                            processing  = undefined
                            return result
                        })
                }
                return processing.inproc
            }
            ,createContext: function(target){
                return processContext({
                    target: target
                })
            }
            ,addQueue: function(queue) {
                this.queues.push(queue)
                return this
            }
        })

    })

return model
}

