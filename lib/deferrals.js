    ...
    .enclose(function(){
        ...
        function handleScheduled(context, inputType, args, resolve) {
            return context.once(EVENTS.CONTEXT.COMPLETED, function(e){
                debug('schedule-deferral','context',context.id,'handling','scheduled input:',inputType)
        ...
        stampit.mixIn(this,{
            ...
            schedule: function(inputType, args) {
                ...
                if(context && context.is(EVENTS.CONTEXT.PROCESSING)) {
                    debug('schedule-deferral','context',context.id,'scheduling input:',inputType)
                    context.once(EVENTS.CONTEXT.COMPLETED, function(e) {
                        debug('schedule-deferral','context',context.id,'handling','scheduled input:',inputType)
                        return this.handle(inputType,args)
                    }.bind(this))
                    return Promise.resolve(this)
                }
                debug('schedule-deferral','immediately handling input:',inputType)
                return this.handle(inputType,args)
            }
        })
    })
