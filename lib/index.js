        ...
        .enclose(function(){
            ...
            var stateful = stampit()
                ...
                .methods({
                    ...
                    ,collect: function(inputType, context) {
                        var handlers = []
                        ...
                        if(inputType === '_start') {
                            push.call(handlers, handleStart.bind(context || this))
                        }
                        if(inputType === '_transition') {
                            push.call(handlers, handleTransition.bind(context || this))
                        }
                        var current = this.config(context.state)

                        if(current && current[inputType]){
                            var handler = current[inputType]
                            push.call(handlers, current[inputType].bind(context || this))
                        }
                        return handlers
                    }
                })
            ...
            states = stateful({ states: this.states, when: this.when })
            ...
            function collect(inputType) {
                ...
                if(inputType === '_start') {
                    push.call(handlers, handleStart.bind(this))
                }
                if(inputType === '_transition') {
                    push.call(handlers, handleTransition.bind(this))
                }
                var current = this.states[this.state]
                if(current && current[inputType]){
                    push.call(handlers, current[inputType])
                }
                return handlers
            }
