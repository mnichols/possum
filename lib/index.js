...
.init(function(){
    ...
    const emittable =  stampit()
        ...
        .init(function(){
            //expose emitter
            this.emitter = (cfg.emitter || new EventEmitter(cfg.emitterOpts))
    ...
    const builder = stampit()
        .init(function(){
            ...
            /**
             * compose the resulting machine with these behaviors
             * */
            this.compose = (...these) => {
            ...
            /**
             * proxy to underlying props method on machine
             * */
            this.props = (obj) => {
                apiModel = apiModel.props( obj )
                return this
            }
            /**
             * proxy to underlying props method on machine
             * */
            this.refs = (obj) => {
                apiModel = apiModel.refs( obj )
                return this
            }

            /**
             * return underlying machine stamp. Note that `target` will
             * not be set on this even if `target` was assigned earlier.
             * Also note that emitter options will not be passed into
             * instances created with this so you that burder is on the caller
             * to pass options if needed.
             * @return {stamp} api model stamp
             * */
            this.stamp = () => {
                return apiModel.props({
                    states: states
                })
            }
