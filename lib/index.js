    ...
    var evented = stampit()
        .enclose(function(){
            ...
            stampit.mixIn(this,{
                emitter: emitter
                /**
                 * utility for namespacing `str` for this possum instance
                 * */
                ,namespaced: function(str, namespace) {
                    if(!str) {
                        //getter
                    ...
                    return ((namespace || this.namespace) + delimiter + str)
                }
            })
            var api = [
                'on'
            ...
            emitter.on(EVENTS.QUEUE.HANDLED,function(e) {
                var message = e.message
                    ,action = e.action
                return raise.call(this, event({
                        topic: this.namespaced(EVENTS.HANDLED)
            ...
            emitter.on(EVENTS.QUEUE.COMPLETED,function(e) {
                var message = e.message
                    ,action = e.action
                return raise.call(this, event({
                        topic: this.namespaced(EVENTS.COMPLETED)
                        ,inputType: message.inputType
                        ,payload: message.payload
                        ,action: action
            ...
            emitter.on(EVENTS.QUEUE.NO_HANDLER,function(e) {
                var message = e.message
                    ,action = e.action
                return raise.call(this, event({
                        topic: this.namespaced(EVENTS.NO_HANDLER)
                        ,inputType: message.inputType
                        ,payload: message.payload
                        ,action: action
                        ,namespace: this.namespace
            ...
            emitter.on(EVENTS.QUEUE.COMPLETED,function(e) {
                ...
                var ev = event({
                    topic: this.namespaced(EVENTS.TRANSITIONED)
                    ,payload: message.payload
                    ,action: action
                    ,namespace: this.namespace
                })

                return raise.call(this, stampit.mixIn(ev,message.payload))
            }.bind(this))
        })
        ...
        .methods({
            transition: function(toState) {
                ...
                if(!this.router().hasState(toState)) {
                    var e = event({
                        topic: this.namespaced(EVENTS.INVALID_TRANSITION)
                        ,state: this.state
                        ,payload: payload
                    })
                    return Promise.resolve(this)
                        .then(raise.bind(this,e))
                        .return(payload)
                }
