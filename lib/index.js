...
function possumBuilder(stamp) {
    ...
    return stampit()
        ...
        .compose(emittable.refs({emitterOpts: stamp.fixed.refs.emitterOpts}))
        .props({
            handlers: states
        })
        .init(function(){
            ...
            this.transition = function(toState) {
                ...
                let enter = this.handlers.getEntry(toState)
        ...
        //being sure to add _last_ so that you can interact with possum methods in your behaviors
        .compose(stamp)
...
.static({
    /**
     * get a possum builder with the current state of the `stamp`
     * to compose in
     * */
    builder: function(){
        return possumBuilder(this)
    }
})
.init(function({stamp}){
    //do nothing here
